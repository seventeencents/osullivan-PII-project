...
However, before we can proceed with either step we first need to ensure that the size of our data is manageable for the scope of our machine. Modeling a very large corpus of unstructured text data incurs high computational costs due to the increasing dimensionality of the data as we begin to structure it. As a result, the amount of data that can be modeled will vary significantly based on the computational resources available to you. 

At this time we've made the decision to examine a 10% sample of our overall data set for the purpose of modeling on our machine, but the proportion parameter for the subsample partition can be tweaked according to your available resources. 

```{r}
# create smaller partition for the sake of being able
...
# create index of train partition
...
gridXG <- expand.grid(nrounds = c(100, 200),
                      max_depth = c(3, 6),
                      eta = c(0.1, 0.3),
                      gamma = c(0, 0.1),
...
projSVD <- function(type, trainirlba, testtfidf) {
  ...
  if (type == "bod") {
    ...
    names(projection) <- paste0("Z", seq(1,300))
  }
  return(projection)
}
...
train.sub <- as.data.frame(train.sub.irlba$v)
train.sub <- train.sub %>%
  rename_all(~ paste0("Z", seq(1, length(names(train.sub)))))
```
...
```{r}
###### Body Tokens Only ######
# tune an XGBoost model with body token SVD
xg.body.svd <- customXG(train.body.svd)

saveRDS(xg.body.svd, file = here("results", "large-files", "xg-body-svd.rds"))

# tune an XGBoost model with body token SVD and email length
xg.body.svd.length <- customXG(train.body.svd.length)

saveRDS(xg.body.svd.length, file = here("results", "large-files", "xg-body-svd-length.rds"))

### can't be run in a reasonable timeframe
### too computationally intensive for a MacBook
# tune an XGBoost model with body token SVD, email length, and sender domain
#xg.body.svd.length.sender <- customXG(train.body.svd.length.sender)

#saveRDS(xg.body.svd.length.sender, file = here("results", "large-files", "xg-body-svd-length-sender.rds"))

###### Subject Tokens Only ######
# tune an XGBoost model with subject token SVD
xg.sub.svd <- customXG(train.sub.svd)

saveRDS(xg.sub.svd, file = here("results", "large-files", "xg-sub-svd.rds"))

# tune an XGBoost model with subject token SVD and email length
xg.sub.svd.length <- customXG(train.sub.svd.length)

saveRDS(xg.sub.svd.length, file = here("results", "large-files", "xg-sub-svd-length.rds"))

### can't be run in a reasonable timeframe
### too computationally intensive for a MacBook
# tune an XGBoost model with subject token SVD, email length, and sender domain
#xg.sub.svd.length.sender <- customXG(train.sub.svd.length.sender)

#saveRDS(xg.sub.svd.length.sender, file = here("results", "large-files", "xg-sub-svd-length-sender.rds"))

###### All Tokens ######
# tune an XGBoost model with all token SVD
xg.all.svd <- customXG(train.all.svd)

saveRDS(xg.all.svd, file = here("results", "large-files", "xg-all-svd.rds"))

# tune an XGBoost model with all token SVD and email length
xg.all.svd.length <- customXG(train.all.svd.length)

saveRDS(xg.all.svd.length, file = here("results", "large-files", "xg-all-svd-length.rds"))

### can't be run in a reasonable timeframe
### too computationally intensive for a MacBook
# tune an XGBoost model with all token SVD, email length, and sender domain
#xg.all.svd.length.sender <- customXG(train.all.svd.length.sender)

#saveRDS(xg.all.svd.length.sender, file = here("results", "large-files", "xg-all-svd-length-sender.rds"))
```
...
test.body.svd.proj <- projSVD("bod", train.body.irlba, test.body.tokens.tfidf)
```
...
```{r}
# project the SVD from the train split onto the test split
test.sub.svd.proj <- projSVD("sub", train.sub.irlba, test.sub.tokens.tfidf)
```

## Final Data Frames for Predictions

Having now completed the transformations necessary to map the test data into the same space as the training data used for modeling we can proceed with making predictions and assessing model performance. The latter of which will be continued in a second `statistical-analysis-continued.qmd` file using the pertinent, saved `.rds` files from this document. These files have been excluded from any GitHub commits, because of the generous sizes of each model's data file. In order to proceed with the next notebook you'll need to generate the model files on your local machine first.
