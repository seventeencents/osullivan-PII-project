...
customRF <- function(df) {
  cores <- detectCores()
  clus <- makeCluster(cores[1] - 1, type = "SOCK") # determine the optimal number
...
#rf.body.preds <- predict(rf.body, test.body.svd)
#rf.body.length.preds <- predict(rf.body.length, test.body.svd.length)
#rf.body.length.sender.preds <- predict(rf.body.length.sender, test.body.svd.length.sender)
```
