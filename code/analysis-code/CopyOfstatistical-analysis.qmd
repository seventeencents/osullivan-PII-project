...
sub.svd <- as.data.frame(train.sub.irlba$v)
sub.svd <- sub.svd %>%
  rename_all(~ paste0("Z", seq(1, length(names(sub.svd)))))
```
...
confusionMatrix(xg.body.svd.preds, test.body.svd$label)
xg.all.svd.length.preds <- predict(xg.all.svd.length, test.all.svd.length)
confusionMatrix(xg.all.svd.length.preds, test.body.svd$label)
```
